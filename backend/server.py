...
# Load model and label encoder with improved error handling
def load_resources():
    global model, label_encoder
    try:
        model = tf.keras.models.load_model(MODEL_PATH, compile=False)  # Avoid recompiling if not needed
...
def predict():
    ...
    # Check for features in form data
    ...
    try:
        # Parse features: support both JSON array and comma-separated string
    ...
    # Adjust feature shape for model input
    try:
        if len(features.shape) == 1:
            features = np.expand_dims(features, axis=0)  # Add batch dimension if missing
        expected_features = 63  # Adjust this based on your model's expected input size
    ...
    try:
        ...
        predicted_class_index = np.argmax(predictions, axis=-1)[0]  # Handle batch dimension
        confidence = float(np.max(predictions, axis=-1)[0])  # Convert to float for JSON serialization
